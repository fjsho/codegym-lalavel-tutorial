...
class PictureStoreTest extends TestCase
{
    ...
    public function jpg画像を投稿できる()
    {
        ...
        $referer = route('tasks.edit', ['project' => $project->id, 'task' => $task->id]);

        $response = $this->actingAs($user)
        ->from($referer)
        ->post(route('task_pictures.store', ['project' => $project->id, 'task' => $task->id]),[
            'file' => $file,
        ]);
     ...
     * @test
     */
    public function 指定した容量未満の画像を登録することができる()
    {
        $user = User::factory()->create();
        $project = Project::factory()->create([
            'created_user_id' => $user->id,
        ...
        $max_size_mb = 10.05;
        $file_size_kb = $max_size_mb * 1024 - 1; //指定容量10.5MB（10,500KB未満は保存できる）
     ...
     * @test
     */
    public function 画像は指定枚数まで保存することができる()
    {
        $user = User::factory()->create();
        $project = Project::factory()->create([
            'created_user_id' => $user->id,
        ...
        for ($count=0; $count < $max_count; $count++) { 
            Storage::disk('public')->assertExists($file_names[$count]);
        }
        $response->assertRedirect($referer);
    }
     ...
     * @test
     */
    public function 指定枚数を超えた画像は保存できない()
    {
        $user = User::factory()->create();
        $project = Project::factory()->create([
            'created_user_id' => $user->id,
        ...
        画像枚数のチェックはモデルの処理としておこなっているためエラーの保存場所が異なる。
